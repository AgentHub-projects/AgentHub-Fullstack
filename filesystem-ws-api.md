...
gateway 使用 `sessionId` 找到当前 session 对应的 sandbox，再把 `/filesystem/socket.io` 反代到 sandbox。
...
| 字段 | 位置 | 必填 | 说明 |
|---|---|---:|---|
| `sessionId` | query | 是 | gateway 用它解析当前 session 的 sandbox endpoint |
...
变更应用成功并自动提交后，服务端会主动推送 `fs:changed`。
...
```http
GET /filesystem/download/<path>?sessionId=<sessionId>
```

示例：

```http
GET /filesystem/download/src/App.tsx?sessionId=s1
```
